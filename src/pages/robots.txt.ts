 ...
 *
 * 重要な制約:
 *   robots.txt はオリジンの直下 `/robots.txt` にあるものだけが読まれる（RFC 9309）。
 ...
 *
 *   いまはプロジェクトサイトで配信しているので、**このファイルは効いていない。**
 *   クロール自体は止まっていない（robots.txt が無いオリジンは全許可と解釈される）。
 *   回避方法は docs/decisions/13-origin-root-limitation.md に書いた。
 *   このファイルを消さずに残しているのは、配信先がユーザーサイトか独自ドメインに
 *   変わった時点で、何も直さずに効き始めるようにしておくため。
 *
 * 方針:
